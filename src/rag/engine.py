...
from src.rag.category_manager import CategoryManager
...
class RagEngine:
    def __init__(
        ...
        self.category_manager = CategoryManager(
            logger, self.file_handler, cryptocompare_api, symbol_manager, self.index_manager
        )
    ...
    @property
    def news_database(self) -> List[Dict[str, Any]]:
        """Get news database from news manager"""
        return self.news_manager.news_database

    def get_known_tickers(self) -> Set[str]:
        """Get known crypto tickers from category manager"""
        return self.category_manager.get_known_tickers()

    def get_current_market_overview(self) -> Optional[Dict[str, Any]]:
        """Get current market overview from market data manager"""
        return self.market_data_manager.get_current_overview()
    
    # Additional delegation methods for backwards compatibility
    def search_by_category(self, category: str) -> List[int]:
        """Search for articles by category"""
        return self.index_manager.search_by_category(category)
    ...
    def search_by_coin(self, coin: str) -> List[int]:
        ...
        return self.index_manager.search_by_coin(coin)
    
    async def update_market_overview_if_needed(self) -> None:
        """Update market overview if needed"""
        await self.market_data_manager.update_market_overview_if_needed()
    
    def get_database_size(self) -> int:
        """Get size of news database"""
        return self.news_manager.get_database_size()
    
    def get_category_word_map(self) -> Dict[str, str]:
        """Get category word mapping"""
        return self.category_manager.get_category_word_map()
