...
class MarketDataCollector:
    ...
    def _process_market_data(self, fear_greed_data: List[Dict[str, Any]], context) -> List:
        """Process raw market data with sentiment information"""
        ...
        for item in fear_greed_data:
            ...
            historical_sentiments.append({
                'timestamp': datetime.fromtimestamp(timestamp),
                'value': int(item["value"]),
                'value_classification': item["value_classification"]
            })
            
            fear_greed_map[timestamp] = {
                "value": int(item["value"]),
                "classification": item["value_classification"]
            }
        
        # Use the extract_ohlcv_data method to avoid redundant code
        data = self.extract_ohlcv_data(context)
        
        # Now add sentiment to each data point
        for market_data in data:
            ...
            # Add historical data to the first sentiment object we find
            if sentiment and market_data == data[0] and historical_sentiments:
                sentiment['historical'] = historical_sentiments
                
            market_data['sentiment'] = sentiment
        
        return data
        
    def _process_sentiment(self, day_timestamp: int, 
                         fear_greed_map: Dict[int, Dict[str, Any]]) -> Dict:
        """Process sentiment data for a specific day"""
        sentiment = None
        for fg_timestamp, fg_data in fear_greed_map.items():
            if abs(fg_timestamp - day_timestamp) < 86400:
                sentiment = dict(
